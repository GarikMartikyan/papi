...
/**
 * Короткое имя панели — её же аббревиатура строчными: `RMP` даёт `rmp`.
 *
 * Отдельно от `getAppAbbr`, потому что это не подпись, а ключ: под ним панель
 * записана в каталоге логотипов ядра, и по нему `PanelLogo` без пропсов находит
