 ...
 * (`injectSlices` для слайсов, `configureApi` + `injectEndpoints` для запросов).
 */
export const store = configureStore({
  reducer: rootReducer,
  // Единственное место, где регистрируется middleware baseApi: после создания
  // стора его уже не добавить, поэтому панель и не создаёт свой api.
